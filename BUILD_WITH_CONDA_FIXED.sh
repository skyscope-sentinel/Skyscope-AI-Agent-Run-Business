...
# Ultimate Enterprise Suite Builder v1.0.0 (CONDA FIXED EDITION)
# ====================================
...
# This version uses Conda with Python 3.11 for maximum compatibility.
...
# Function to verify we're in the conda environment
verify_conda_env() {
    if [[ -z "$CONDA_PREFIX" || "$CONDA_PREFIX" != *"$env_name"* ]]; then
        handle_error "Not in the correct conda environment. Exiting." "fatal"
    fi
}

# Function to handle conda package installation with error recovery
install_conda_package() {
    ...
    if [ -z "$skip_reason" ]; then
        if [ -z "$channel" ]; then
            log_info "Installing $package with conda..."
            if conda install -y $package; then
                ...
                if [ -n "$alt_package" ]; then
                    ...
                    if conda install -y $alt_package; then
            ...
            log_info "Installing $package from $channel channel..."
            if conda install -y -c $channel $package; then
                log_success "$package installed successfully from $channel channel"
                return 0
            else
                if [ -n "$alt_package" ]; then
                    log_warning "Failed to install $package. Trying alternative: $alt_package"
                    ...
                    log_error "Failed to install $package from $channel channel. Continuing..."
                    return 1
                fi
            fi
        fi
...
# Function to handle pip package installation with error recovery
install_pip_package() {
    package=$1
    alt_package=$2
    skip_reason=$3
    ...
    if [ -z "$skip_reason" ]; then
        log_info "Installing $package with pip (not available in conda)..."
...
echo -e "${MAGENTA}  ╚══════╝╚═╝  ╚═══╝   ╚═╝   ╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝  ╚═╝╚═╝╚══════╝╚══════╝${NC}"
echo ""
echo "                      Ultimate Enterprise Suite Builder v1.0.0 (CONDA FIXED EDITION)"
echo ""
echo "                      ===================================="
echo ""
...
echo ""
echo "The setup will:"
echo " - Remove any existing venv directory to avoid conflicts"
echo " - Install Miniconda if not present"
echo " - Create a conda environment with Python 3.11"
echo " - Install all required dependencies using conda whenever possible"
...
echo "This version uses Conda with Python 3.11 for maximum compatibility."
echo ""
...
log_info "Checking for existing venv directory..."
if [ -d "$base_dir/venv" ]; then
    log_warning "Found existing venv directory. Removing to avoid conflicts..."
    rm -rf "$base_dir/venv"
    log_success "Existing venv directory removed"
fi
...
log_section "Setting Up Miniconda"
...
if command_exists conda; then
    ...
    if [[ "$is_apple_silicon" == true ]]; then
        curl -L https://repo.anaconda.com/miniconda/Miniconda3-latest-MacOSX-arm64.sh -o "$miniconda_installer"
    else
        curl -L https://repo.anaconda.com/miniconda/Miniconda3-latest-MacOSX-x86_64.sh -o "$miniconda_installer"
    fi
...
log_section "Creating Conda Environment"

env_name="skyscope"
log_info "Creating conda environment '$env_name' with Python 3.11..."
...
if conda create -y -n "$env_name" python=3.11; then
    ...
    handle_error "Failed to create conda environment" "fatal"
fi

# Activate the environment using source to ensure it works in the current shell
eval "$(conda shell.bash hook)"
conda activate "$env_name" || handle_error "Failed to activate conda environment" "fatal"

# Verify we're in the right environment
python_version=$(python --version)
if [[ "$python_version" != *"3.11"* ]]; then
    handle_error "Wrong Python version: $python_version. Expected Python 3.11. Environment activation failed." "fatal"
fi

log_success "Successfully activated conda environment with $python_version"
...
if command_exists docker; then
    ...
    brew install --cask docker || log_warning "Docker installation failed, but we'll continue"
fi
...
if [ -d "/Applications/Visual Studio Code.app" ]; then
    ...
    brew install --cask visual-studio-code || log_warning "VS Code installation failed, but we'll continue"
fi

# Verify conda environment again after Homebrew operations
verify_conda_env

# Install conda packages
log_section "Configuring Conda Channels"
...
# Update conda-forge
log_info "Updating conda-forge..."
conda update -y -c conda-forge --all

log_section "Installing Core Scientific Packages via Conda"

# Install basic packages from conda-forge
log_info "Installing basic scientific packages from conda-forge..."
conda install -y -c conda-forge numpy pandas matplotlib seaborn scikit-learn || log_warning "Some basic packages failed to install"

log_section "Installing Cython and Compiled Packages"

# Install Cython explicitly first
log_info "Installing Cython from conda-forge..."
conda install -y -c conda-forge cython || handle_error "Failed to install Cython" "fatal"

# Install cytoolz from conda-forge (avoids the Cython compilation issues)
log_info "Installing cytoolz from conda-forge..."
conda install -y -c conda-forge cytoolz || handle_error "Failed to install cytoolz" "fatal"

# Install toolz from conda-forge
log_info "Installing toolz from conda-forge..."
conda install -y -c conda-forge toolz || log_warning "Failed to install toolz"

log_section "Installing TensorFlow"

# Install TensorFlow
if [[ "$is_apple_silicon" == true ]]; then
    log_info "Installing TensorFlow for Apple Silicon..."
    pip install tensorflow-macos tensorflow-metal || log_warning "TensorFlow for Apple Silicon installation failed"
else
    log_info "Installing TensorFlow for Intel Mac..."
    conda install -y -c conda-forge tensorflow || log_warning "TensorFlow installation failed"
fi

log_section "Installing GUI Components"

# Verify conda environment again before GUI installation
verify_conda_env

# Install PyQt6 with pip (not well supported in conda)
log_info "Installing PyQt6 with pip..."
pip install PyQt6 PyQt6-Charts || log_warning "GUI packages installation failed"

log_section "Installing AI Provider Packages"

# Verify conda environment again before AI packages installation
verify_conda_env

# Install AI provider packages with pip (not available in conda)
log_info "Installing AI provider packages with pip..."
pip install openai-unofficial google-generativeai huggingface_hub anthropic || log_warning "AI provider packages installation failed"

log_section "Installing Blockchain and Finance Packages"

# Verify conda environment again before blockchain installation
verify_conda_env

# Install web3.py and other blockchain tools
log_info "Installing web3.py and blockchain tools..."
conda install -y -c conda-forge web3 || pip install web3
pip install pycoingecko python-binance || log_warning "Some blockchain packages failed to install"

# Install eth-brownie prerequisites from conda-forge
log_info "Installing eth-brownie prerequisites from conda-forge..."
conda install -y -c conda-forge eth-utils eth-abi eth-keys eth-typing hexbytes || log_warning "eth-brownie prerequisites installation failed"

# Install additional eth-brownie dependencies
log_info "Installing additional eth-brownie dependencies..."
conda install -y -c conda-forge pycryptodome || pip install pycryptodome
conda install -y -c conda-forge pyrsistent || pip install pyrsistent
conda install -y -c conda-forge bitarray || pip install bitarray

# Install eth-brownie with pip after all prerequisites are installed
log_info "Installing eth-brownie with pip..."
pip install eth-brownie==1.19.3 || log_warning "eth-brownie installation failed, but we'll continue"

log_section "Installing Development Tools"

# Verify conda environment again before dev tools installation
verify_conda_env

# Install development tools from conda-forge
log_info "Installing development tools from conda-forge..."
conda install -y -c conda-forge pytest black mypy flake8 autopep8 isort || log_warning "Development tools installation failed"

# Set up Pinokio with error handling
log_section "Setting Up Pinokio"

# Verify conda environment again before Pinokio setup
verify_conda_env

pinokio_dir="$base_dir/pinokio"
if [ -d "$pinokio_dir" ]; then
    log_info "Pinokio directory already exists. Reinitializing..."
    # Check if it's a git repository
    if [ -d "$pinokio_dir/.git" ]; then
        cd "$pinokio_dir"
        git pull || log_warning "Failed to update Pinokio repository, but we'll continue"
        cd - > /dev/null
    else
        log_warning "Existing Pinokio directory is not a git repository. Removing and reinitializing..."
        rm -rf "$pinokio_dir"
...
# Verify conda environment again before config generation
verify_conda_env
...
# Verify conda environment again before creating application files
verify_conda_env
...
# Get conda environment name from config
ENV_NAME=\$(grep -o '"conda_env": "[^"]*"' "\$CONFIG_FILE" | cut -d'"' -f4)
if [ -z "\$ENV_NAME" ]; then
    ENV_NAME="skyscope"
fi
...
PYTHON_VERSION=\$(python --version)
if [[ "\$PYTHON_VERSION" != *"3.11"* ]]; then
    echo "Error: Wrong Python version: \$PYTHON_VERSION. Expected Python 3.11."
...
# Verify conda environment again before creating application bundle
verify_conda_env
...
# Get conda environment name from config
CONFIG_FILE="\$HOME/SkyscopeEnterprise/config/config.json"
ENV_NAME=\$(grep -o '"conda_env": "[^"]*"' "\$CONFIG_FILE" | cut -d'"' -f4)
if [ -z "\$ENV_NAME" ]; then
    ENV_NAME="skyscope"
fi
...
PYTHON_VERSION=\$(python --version)
if [[ "\$PYTHON_VERSION" != *"3.11"* ]]; then
    osascript -e 'display dialog "Error: Failed to activate conda environment. Expected Python 3.11 but got '"$PYTHON_VERSION"'" buttons {"OK"} default button "OK" with icon stop with title "Skyscope Enterprise Suite"'
    exit 1
fi
...
# Verify conda environment again before creating API key setup script
verify_conda_env
...
echo ""
echo "The Skyscope Enterprise Suite has been successfully built with Conda!"
echo ""
echo -e "${GREEN}Python version:${NC} $(python --version)"
echo -e "${GREEN}Conda environment:${NC} $env_name"
echo ""
